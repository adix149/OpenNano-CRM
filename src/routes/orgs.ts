...
import { db } from "../db";
import { orgs, projects, entities } from "../db/schema";
...
app.post("/", async (c) => {
  ...
  const [exists] = await db.select().from(orgs).where(eq(orgs.slug, parsed.data.slug));
  if (exists) return c.json({ error: "Org slug already exists" }, 409);
  const [org] = await db.insert(orgs).values({ slug: parsed.data.slug, name: parsed.data.name, description: parsed.data.description ?? null }).returning();
...
app.patch("/:id", async (c) => {
  ...
  const [current] = await db.select().from(orgs).where(eq(orgs.id, id));
  ...
  if (newSlug && newSlug !== current.slug) {
    const [taken] = await db.select({ id: orgs.id }).from(orgs).where(eq(orgs.slug, newSlug));
  ...
  try {
    const [updated] = await db.update(orgs).set(parsed.data as any).where(eq(orgs.id, id)).returning();
...
projectApp.get("/:id", async (c) => {
  ...
  const ents = await db.select().from(entities).where(eq(entities.projectId, id));
  return c.json({ ...proj, entities: ents });
});
